...
The global [@GlobalScope.typeof()](@GlobalScope.md#@GlobalScope_method_typeof) function returns the enumerated value of the Variant type stored in the current variable (see [Variant.Type](@GlobalScope.md#enum_@GlobalScope_Variant.Type)).
...
\ **Containers (Array and Dictionary):** Both are implemented using variants. A [Dictionary](Dictionary.md) can match any datatype used as key to any other datatype. An [Array](Array.md) just holds an array of Variants. Of course, a Variant can also hold a [Dictionary](Dictionary.md) and an [Array](Array.md) inside, making it even more flexible.

Modifications to a container will modify all references to it. A [Mutex](Mutex.md) should be created to lock it if multi-threaded access is desired.
